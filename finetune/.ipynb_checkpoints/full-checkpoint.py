...
max_seq_length = 512
#max_seq_length = 20
#epoch_size = 60  # train dataset size
#epoch_size = 798
epoch_size = 1114
num_epochs = 5
...
#weight_decay = 0.02
weight_decay = 0.0
# added from FBK block_size and warmup_iters (it seems to be unused)
warmup_iters = 100
...
def setup(
    ...
    MLFLOW_S3_ENDPOINT_URL='http://192.168.3.78:9100'
    logger = CSVLogger(out_dir.parent, out_dir.name, flush_logs_every_n_steps=log_interval)
    mlf_logger = MLFlowLogger(experiment_name="clinkart_llama7B", tracking_uri=MLFLOW_TRACKING_URI)
    fabric = L.Fabric(devices=fabric_devices, strategy=strategy, precision=precision, loggers=[logger, mlf_logger])
...
def train(
    ...
    model.config.block_size = 512 #(from FBK hyperparameters)
    fabric.print(
    ...
    for iter_num in range(1, max_iters + 1):
        ...
        with fabric.no_backward_sync(model, enabled=is_accumulating):
            ...
            loss = chunked_cross_entropy(logits[..., :-1, :], targets[..., 1:], chunk_size=47)
...
def validate(fabric: L.Fabric, model: GPT, val_data: List[Dict], tokenizer: Tokenizer, max_iters: int) -> torch.Tensor:
    ...
    for k in range(max_iters):
        ...
        logits = model(input_ids)
        losses[k] = chunked_cross_entropy(logits[..., :-1, :], targets[..., 1:], chunk_size=47)
