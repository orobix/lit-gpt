...
def create_balanced_batch(
    data: Sequence[Dict[str, Any]], balance_cfg: dotdict[str, Any], batch_size: int
) -> Sequence[Dict[str, Any]]:
    # retrieves the indices of the elements of the specified clesses and the other classes in data
    idx_specified_classes = np.array(
        [i for i in range(len(data)) if data[i]["class"] in balance_cfg.classes]
    )
    idx_other_classes = np.array(
        [i for i in range(len(data)) if data[i]["class"] not in balance_cfg.classes]
    )

    # If you are unmbalancing the dataset instead of balancing it, an error is raised
    if balance_cfg.percentage < len(idx_specified_classes) / len(data):
        raise ValueError(
            f"Percentage you specified ({balance_cfg.percentage}) is lower than "
    ...
    el_per_specified_classes = int(batch_size * balance_cfg.percentage)
...
def tokenize_dataset(
    fabric: L.Fabric, tokenizer: Tokenizer, dataset_filepath: str
) -> List[Dict[str, Any]]:
    with open(dataset_filepath, "r") as f:
        dataset = json.load(f)

    for sample in dataset:
        prompt = {"instruction": sample["instruction"], "input": sample["input"]}
        prompt_to_encode = generate_prompt(prompt)
        encoded = tokenizer.encode(prompt_to_encode, device=fabric.device)
        sample["encoded_prompt"] = encoded
