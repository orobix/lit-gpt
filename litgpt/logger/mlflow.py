...
class CustomMLFlowLogger(MLFlowLogger):
    ...
    def log_metrics(
        ...
        self.experiment.log_batch(run_id=self.run_id, metrics=metrics_list, synchronous=self.synchronous)
    ...
    def log_hyperparams(self, params: Union[Dict[str, Any], Namespace]) -> None:  # type: ignore[override]
        ...
        for idx in range(0, len(params_list), 100):
            self.experiment.log_batch(
                run_id=self.run_id, params=params_list[idx : idx + 100], synchronous=self.synchronous
            )
